...
contract BirthdayPayout {
    ...
    function addTeammate(
        ...
        emit NewTeammate(account, name, isBirthday);
    }
    ...
    function getDate(uint256 timestamp)
        public
        view
        returns (
            uint256 year,
    ...
    function birthdayPayoutAll() public  onlyOwner {
        // send some money to the teammate
        for (uint256 i = 0; i < _teammates.length; i += 1) {
            if (checkBirthday(i) == true && _teammates[i].isBirthday == false) {
                _teammates[i].isBirthday = true;
                sendToTeammate(i);
                emit HappyBirthday(_teammates[i].name, _teammates[i].account);
            } else if (checkBirthday(i) == false && _teammates[i].isBirthday == true){
                _teammates[i].isBirthday = false;
            }
        }
        // and emit a HeppyBirthday event(just in case)
    }
    ...
    event NewTeammate(address account, string name, bool isBirthday);
